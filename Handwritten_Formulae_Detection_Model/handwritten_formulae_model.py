...
from handwritten_model_helper import getCoordinatesDictList, getSortedFilenames, getModifiedImagePath, getModifiedAnnotationsPath
    ...
    try:
        ...
        for e,result in enumerate(regionProposals):
            ...
            if e < 2000:
                ...
                # With a max of 30 such inputs per input image
                elif maxLoopIOU < 0.05 and falseCounter < 30:
                    # Getting the subset of image with bounding box of the coordinates mentioned
                    timage = imout[y: y + h, x: x + w]
                    # Resizing image
                    resized = cv2.resize(timage, (224, 224), interpolation = cv2.INTER_AREA)
...
finalModel.compile(loss = keras.losses.categorical_crossentropy, optimizer = opt, metrics=["accuracy"])
finalModel.summary()
